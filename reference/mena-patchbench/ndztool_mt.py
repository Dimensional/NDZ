#!/usr/bin/env python3
"""
*** NOT Mena's file. A local, unofficial performance experiment on top of the real
*** ndztool.py (kept byte-for-byte untouched alongside this one) - process-parallel
*** packing via ProcessPoolExecutor, added by the NDZ .NET port project, 2026-09-01.
*** Format logic is unchanged from the original; only pack_ndz_blob's block-compression
*** loop is reorganized to run across worker processes (mirroring pack.rs's own rayon
*** per-thread-group approach, and this port's own Parallel.For). Verified to produce
*** byte-identical .ndz output to the original single-threaded ndztool.py on the same
*** input before being trusted for any real comparison - see the project's memory/docs
*** for that verification. Never treat this file as a reference source; ndztool.py is.
...
# --------------------------------------------------------- multiprocess pack ---
# Real parallelism, not just threads: the per-block Python-level bookkeeping (grain
# hashing, candidate-window offset math, buffer stitching) runs under the GIL same as
# any other pure-Python code, so only separate PROCESSES actually run it concurrently -
# threads would only have parallelized the zstd calls themselves, which aren't this
# workload's bottleneck (see the NDZ .NET port project's own measurements: ~7.5x wall
# time difference against its own multithreaded packer on the identical input, far more
# than the compression calls alone could explain). Each worker builds its own
# compressor state ONCE via the pool's initializer (not per task, and not by pickling
# zstd objects across the process boundary - those aren't meant to be shared that way),
# then processes a contiguous range of whole frames, mirroring pack.rs's own rayon
# per-thread-group chunking (and this port's own Parallel.For over frames).

_mt_worker_state = {}


def _mt_init_worker(data, frame_dsize, block_dsize, level, filters_enabled,
                    raw_dict_bytes, base_data):
    plain_cctx, dict_cctx, _dict_wl = make_block_compressors(level, block_dsize, None)
    raw_cctx = make_rawdict_cctx(raw_dict_bytes, level) if raw_dict_bytes else None
    base_ctx = BaseCtx(base_data, level) if base_data is not None else None
    _mt_worker_state.update(
        data=data, frame_dsize=frame_dsize, block_dsize=block_dsize,
        plain_cctx=plain_cctx, dict_cctx=dict_cctx, raw_cctx=raw_cctx,
        filters_enabled=filters_enabled, base_ctx=base_ctx)


def _mt_compress_frame_range(frame_indices):
    """Same per-frame logic as build_seekable_v2_adv's loop body, against this
    worker's own process-local compressor state - returns one (index, payload, dsize,
    mode_hist, n_windowed) tuple per frame in frame_indices, not stitched together,
    since frames from different workers arrive out of order and must be re-sorted by
    the caller before concatenation."""
    w = _mt_worker_state
    data, frame_dsize, block_dsize = w["data"], w["frame_dsize"], w["block_dsize"]
    plain_cctx, dict_cctx, raw_cctx = w["plain_cctx"], w["dict_cctx"], w["raw_cctx"]
    filters_enabled, base_ctx = w["filters_enabled"], w["base_ctx"]

    out = []
    for fi in frame_indices:
        fstart = fi * frame_dsize
        fchunk = data[fstart:fstart + frame_dsize]
        sub_blocks, sub_modes, sub_offs, sub_stats = [], [], [], []
        mode_hist = [0] * 7
        n_windowed = 0
        ...
        frame_payload = bytes(index_hdr) + b"".join(sub_blocks)
        out.append((fi, frame_payload, len(fchunk), mode_hist, n_windowed))
    return out


def build_seekable_v2_adv_mt(data, frame_dsize, block_dsize, level, filters_enabled,
                             base_data, raw_dict_bytes, workers, progress=None):
    """Same computation and output as build_seekable_v2_adv (dict_obj/trained-dict
    support intentionally dropped - pack_ndz_blob's own real call site never uses it
    either, see build_ndz_frontmatter's remarks elsewhere), spread across worker
    processes. base_data/raw_dict_bytes are raw bytes, not the zstd context objects -
    each worker reconstructs its own compressor state from them once, not per frame.
    """
    n_frames = (len(data) + frame_dsize - 1) // frame_dsize
    if n_frames == 0:
        table = struct.pack("<II", 0, LZ4BENCH_MAGIC)
        return table, [], [], [0] * 7, None, 0

    workers = max(1, min(workers, n_frames))
    group_len = (n_frames + workers - 1) // workers
    groups = [list(range(i, min(i + group_len, n_frames)))
             for i in range(0, n_frames, group_len)]
    ...
    with ProcessPoolExecutor(
        max_workers=len(groups), initializer=_mt_init_worker,
        initargs=(data, frame_dsize, block_dsize, level, filters_enabled,
                 raw_dict_bytes, base_data),
    ) as ex:
        for group_result in ex.map(_mt_compress_frame_range, groups):
            for fi, payload, dsize, fmh, fnw in group_result:
                results[fi] = (payload, dsize)
                for i in range(7):
                    mode_hist[i] += fmh[i]
                n_windowed += fnw
                done_frames += 1
                if progress is not None and (done_frames & 7) == 0:
                    progress(done_frames * frame_dsize, len(data))

    out = bytearray()
    frame_stats = []
    for fi in range(n_frames):
        payload, dsize = results[fi]
        out.extend(payload)
        frame_stats.append(BlockStat(csize=len(payload), dsize=dsize))
    ...
    table += struct.pack("<II", len(frame_stats), LZ4BENCH_MAGIC)
    out.extend(table)
    # per_frame_blocks (full per-block detail - offsets/csizes/dsizes for every single
    # block) isn't reconstructed here since nothing downstream needs the actual content:
    # pack_ndz_blob's own info dict only calls len() on each frame's entry (for
    # n_blocks), so each placeholder just needs to be the right LENGTH, not real data.
    per_frame_blocks_placeholder = [
        [None] * ((s.dsize + block_dsize - 1) // block_dsize) for s in frame_stats
    ]
    return bytes(out), frame_stats, per_frame_blocks_placeholder, mode_hist, None, n_windowed
...
def pack_ndz_blob(nds_data, block_size, frame_size, level, filters_enabled,
    ...
    # workers<=1 goes through the ORIGINAL, unmodified single-threaded function -
    # identical code path to the real ndztool.py, zero behavioral risk. Only
    # workers>1 takes the multiprocess path (see build_seekable_v2_adv_mt's remarks).
    if workers <= 1:
        ...
        seekable, frame_stats, per_frame_blocks, mode_hist, _wl, n_windowed = \
            build_seekable_v2_adv_mt(nds_data, frame_size, block_size, level,
                                     bool(flags & NDZ_FLAG_FILTERS),
                                     base_data if base_ctx is not None else None,
                                     raw_dict if raw_cctx is not None else None,
...
"""
    ...
    p.add_argument("--workers", type=int, default=1,
                   help="[ndztool_mt.py only, not in the real ndztool.py] pack across "
                        "this many worker PROCESSES (default 1 = original single-"
                        "threaded behavior); os.cpu_count() is a reasonable choice")
